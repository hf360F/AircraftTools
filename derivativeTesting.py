...
if True:
    ...
    dorsalxstart = 4 # m, at fairing tip
    dorsalxend = 28 # m, at fairing tip
    tankDiameter = 3.0 # m
    dorsalXsecCount = 30
    tank_zoffset = 0 # m
    ...
    def fairingProfile(x, A1=dorsalxstart, A2=dorsalxend, L1=0.4, L2=0.9, S1=2, S2=3):
        ...
        back = (logistic2(x+S2*L2) - logistic2(A2+S2*L2))/(1 + 2*logistic2(A2-S2*L2))
        return np.max((0, tankDiameter*2*(front+back-0.5)))
    
    if True:
        fig2, ax2 = plt.subplots(1, 1, dpi=200)
        xstest = np.linspace(0, dorsalxend+dorsalxstart, 1000)
        ystest = [fairingProfile(x) for x in xstest]
        ...
        # 2:1 elliptical end geometry
        ymax = np.max(ystest)
        ODs = np.linspace(0.9*ymax, 0.99*ymax, 100)
        volumes = []
        ...
        for OD in ODs:
            ...
            anglesC = np.linspace(tangentAngle, np.pi/2, 100)
            crownys = CR*np.cos(anglesC)
            crownxs = CR*np.sin(anglesC) - CR

            anglesK = np.linspace(np.pi, np.pi-tangentAngle, 100)
            knuckleys = OD/2 - KR*(1 + np.cos(anglesK))
            knucklexs = KR*np.sin(anglesK) - 0.25*OD
            ...
            # Place endcaps to just fit within fairing
            heightIndices = np.argwhere(ystest > OD)
            minPos = xstest[heightIndices[0,0]] - OD/4
            maxPos = xstest[heightIndices[-1,0]] + OD/4

            cylinderLength = maxPos - minPos - OD/2
            cylinderVolume = cylinderLength*np.pi*(OD**2)/4

            endVolume = 0.13384358930944257*(OD**3) # Scaling law
            tankVolume = cylinderVolume + 2*endVolume
            volumes.append(tankVolume)

        ax2.plot(-endxs+minPos, endys+OD/2, color="black")
        ax2.plot(endxs+maxPos, endys+OD/2, color="black")
        ax2.plot((minPos+OD/4, maxPos-OD/4), (0, 0), color="black")
        ax2.plot((minPos+OD/4, maxPos-OD/4), (OD, OD), color="black")

        fig, ax3 = plt.subplots(1, 1, dpi=200)
        ax3.plot(ODs, volumes)
        ax3.set_xlabel("Tank assembly diameter, m")
        ax3.set_ylabel("Assembly internal volume, m^3")
        ax3.grid()
